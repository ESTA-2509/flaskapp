...
for k in range(0, count_cate, 1):
        sql_0="SELECT COUNT(`appId`) FROM `apps` GROUP BY `genre` ORDER BY COUNT(`appId`) ASC LIMIT " + str(index) + ", 1"
        ...
        sql_1="SELECT `genre` FROM `apps` GROUP BY `genre` ORDER BY COUNT(`appId`) ASC LIMIT " + str(index) + ", 1"
...
fig.tight_layout()
file_name_8 = 'Top_10_lower.png'
my_path_8 = os.path.abspath(file_name_8)
if os.path.exists(my_path_8):
  os.remove(my_path_8)
plt.savefig(my_path_8, transparent=True)
plt.close('all')